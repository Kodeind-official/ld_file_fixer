...
class _UploadImageSPAPageState extends State<UploadImageSPAPage> {
  ...
  bool get _isFormValid {
    return _selectedDates.every((date) => date != null) &&
           _descriptions.every((price) => price != null && price.isNotEmpty);
  }
...
// }
  ...
  //     // Kompres gambar
  //     final compressedImage =
  //         img.encodeJpg(image, quality: 85); // Sesuaikan kualitas kompresi
  ...
  for (int i = 0; i < _selectedImages.length; i++) {
    try {
      final compressedFile = await compressImage(xFileToFile(_selectedImages[i]));
      if (compressedFile != null) {
        final url = Uri.parse('https://api.verification.lifetimedesign.id/dummy_upload_App_FileFixer.php');
        final request = http.MultipartRequest('POST', url)
          ..files.add(
            await http.MultipartFile.fromPath(
              'image',
          ...
          ..fields['deskripsi'] = _descriptions[i] ?? ''
          ..fields['tanggal'] = _selectedDates[i]?.toIso8601String().split('T')[0] ?? ''
          ..fields['status'] = 'Waiting'
          ..fields['status_user'] = status ?? ''
          ..fields['username'] = username ?? ''
          ..fields['lokasi'] = lokasi ?? ''
  ...
  if (adminStatusList.contains(status)) {
    try {
      final response = await http.get(
        Uri.parse('https://api.verification.lifetimedesign.id/App/Validasi/App_FileFixer/get_admin_users.php'),
      );
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body) as List<dynamic>;
        for (final user in data) {
          ...
          if (adminStatusList.contains(userStatus)) {
            ...
            final headers = {
              'Content-Type': 'application/json',
              'Authorization': 'key=AAAAiAk0jOI:APA91bGHlInl1P3I3QDc0txJFPi8WiwiVFB7gLhSw24pQ34ljqKWHlSy6SkDjuuu4JSZXazb9eYWE1TUzc8DTgZ_7y0gEqCIB6mhNPEGeGuAdtPxg2CNSRQh2iRiSyllK8DI3l31fq2B!',
            };
            final notification = {
              'notification': {
                'title': 'Ada pengajuan baru di terima',
            ...
            if (response.statusCode == 200) {
              ...
              print('Failed to send notification to $username. Status code: ${response.statusCode}');
            }
          }
        }
      } else {
